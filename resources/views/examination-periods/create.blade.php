...
<x-form-layout>
    <h2 class="text-xl font-semibold mb-4">Create New Examination Period</h2>
    <form method="POST" action="{{ route('examination_periods.store') }}">
        @csrf
    
        <!--Start Date -->
        <div>
            <x-input-label for="start_date" :value="'Start Date'" />
            <x-date-input id="start_date" class="block mt-1 w-full"  name="start_date" :value="old('start_date')" required autofocus/>
            <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
        </div>
        <!--End Date -->
        <div>
            <x-input-label for="end_date" :value="'End Date'" />
            <x-date-input id="end_date" class="block mt-1 w-full"  name="end_date" :value="old('end_date')" required autofocus/>
